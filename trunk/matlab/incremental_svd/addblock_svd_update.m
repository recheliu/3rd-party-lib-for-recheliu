...
% TEST-MOD-FROM: [tUp,tSp,tVp] = svds( K, current_rank );

% [tUp, tSp, tVp] = svd( K );
% tUp = tUp(:, 1:current_rank);
% tVp = tVp(:, 1:current_rank);
% tSp = tSp(1:current_rank, 1:current_rank);

K_size = size(K);
if( K_size(1) < K_size(2) )
    K_squared = K * K';
    [tUp, tSp] = pcacov(K_squared);
    sigs = sqrt(tSp);
    n_pos_sigs = sum(sigs>0);
    tVp = K' * tUp;
    tVp(:, 1:n_pos_sigs) = tVp(:, 1:n_pos_sigs) * diag(1.0 ./ sigs(1:n_pos_sigs));
else
    K_squared = K' * K;
    [tVp, tSp] = pcacov(K_squared);
    sigs = sqrt(tSp);
    n_pos_sigs = sum(sigs>0);
    tUp = K * tVp;
    tUp(:, 1:n_pos_sigs) = tUp(:, 1:n_pos_sigs) * diag(1.0 ./ sigs(1:n_pos_sigs));
end
tUp = tUp(:, 1:current_rank);
tVp = tVp(:, 1:current_rank);
tSp = diag(sigs(1:current_rank));
% TEST-MOD-END
