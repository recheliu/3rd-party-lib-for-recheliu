...
// TO:
nrrdLoad(Nrrd *nrrd, const char *_filename, NrrdIoState *nio) {
	char filename[1024];
// MOD-BY-LEETEN 02/24/2011-END
  ...
  airArray *mop;
	...
	// ADD-BY-LEETEN 02/24/2011-BEGIN
  	strcpy(filename, _filename);
	#ifdef WIN32
	{
		char* szToken;
		for(szToken = strchr(filename, '\\'); 
			NULL != szToken;
			szToken = strchr(filename, '\\'))
			*szToken = '/';
	}
	#endif	// #ifdef WIN32
	// ADD-BY-LEETEN 02/24/2011-END
